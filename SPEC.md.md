...
| 姿変えの鍛冶場 | 従者を撃破。相棒、炎を見て一瞬だけ動きが止まる(既視感①・台詞なし、間のみ) |
| なめらかの滝 | 文官を撃破。特に反応なし |
...
| 従者 | 姿変えの鍛冶場 | 「食べれる」「見れる」のように活用を省略する | ら抜き言葉 |
...
- 星のついた問題はマスコットの好物という設定にし、克服(正解し直す)するとマスコットにアクセサリー(装飾)がボーナスで増える
...
**現状(2026年9月時点)**:全8エリアの出題・ストーリー(王座の間の分岐含む)・ふりがな・ステージ再開・操作ガイド・エンディング(称号・クレジット)・ランキング(Firebase、本番動作確認済み)・図鑑拡張・品詞色分け・ホーム画面バッジ・配色フォント適用まで実装済み。Firebase Hostingで公開中(https://paku-kokubunpo.web.app)。GitHub公開を進行中。残る作業は、薄い単元の追加問題データの組み込み、画像・音声本番素材の差し替え、実際の生徒によるプレイテスト。

## 12. 未決事項

- [ ] 黒幕の見た目デザインは確定済み(2章参照)。画像生成のプロンプト化が残タスク
- [ ] 薄い単元の追加出題データ(自動詞他動詞・なめらかの滝・王座の間・丁寧語)をClaude Codeに受け渡し・組み込み
- [ ] 必要なデザイン・音源の一覧の最終確定(13章のたたき台を参照)
- [ ] 実際の生徒(アルバイト先)にMVPを触らせ、最後まで遊び続けられるか・どこで飽きるかを確認する
- [ ] GitHubリポジトリにライセンス(LICENSE)を付けるかどうかの決定
