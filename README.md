...
- **ステージ冒頭の説明**: 出題形式の呼び名と場面(「ことだま使いの心得」など)は、ステージを始めたときに1度だけポップアップで出す(再開では出さない)
- **ことばのずかん(バトル中)**: 出題中の「📖 ずかん」から、いつでも全ページを開いて確認できる(エリアのクリアを待たない)
- **ステージ背景**([StageVisual](src/components/ScreenBackground.tsx)): ボス戦は、上半分に背景+ボスを大きく(`--stage-visual-tall-height`)。通常ステージは、小さな装飾バー(`--stage-visual-height`)にとどめ、問題(特に仕分けのカゴ)の場所を優先する。素材がなければ、エリアの色の仮の帯
