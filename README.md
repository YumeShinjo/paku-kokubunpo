...
## システム文の表記(4章・12章)
ボタン・見出し・案内・通知などのシステム文は、ひらがなだけにせず「漢字+ふりがな」で書く。`"漢字[ふりがな]"` 記法の文字列を [Rb](src/components/Rb.tsx) に渡すと、ふりがな付きで表示される(例: `<Rb t="設定[せってい]" />`)。
エリア名・単元名・小ボスの役職名は、[areaText.ts](src/data/areaText.ts)(`areas.ts` の `nameRuby` / `subBossRuby` と、文法用語の辞書)から、ふりがな付きの表記を引く。正誤の一言([feedbackMessages.ts](src/data/feedbackMessages.ts))・入力の検証メッセージ・ランキングのエラー文も同じ記法。
`aria-label` など読み上げ用の属性は、ふりがなを付けない通常の漢字表記。
...
音量・ミュートの設定は残す。ランキング参加中は、先にクラスから抜けて順位表の自分のデータを消す(通信できないときは、端末のデータだけ初期化し、順位表には残る)。終わると画面を読み込み直す。
