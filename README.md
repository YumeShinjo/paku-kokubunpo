...
- **ミュートとBGM音量0**: どちらも音声セッションを "ambient"(Audio Session API。iOS 16.4以降)にして、効果音だけを、他の音楽アプリを止めずに重ねて鳴らす。BGMは止める。使えない環境では何もしない。アプリはCapacitor等のネイティブ化はしておらず、PWA(Webアプリ)
- **アイコンの設定**は「ランキング」画面(参加フォームと参加後の画面)だけ。「せってい」には置かない。順位表には各人のアイコンが出る
- **ことばのずかん(バトル中)**: 出題中の「📖 ずかん」から、いつでも全ページを開いて確認できる(エリアのクリアを待たない)
- **ステージ背景**([StageVisual](src/components/ScreenBackground.tsx)): ボス戦は、上半分に背景+ボスを大きく(`--stage-visual-tall-height`)。通常ステージは、高さ4.5remの背景バー(`--stage-visual-height`。上端基準で切り出し、下端は背景色へフェード)にとどめ、問題(特に仕分けのカゴ)の場所を優先する。素材がなければ、エリアの色の仮の帯
- **データの引き継ぎ**([transferCode.ts](src/features/transfer/transferCode.ts)): 進み具合は端末(localStorage)にだけ保存している。機種変更・入れ直しのため、「せってい」の「ひきつぎコード」で、進捗・得点・成長・ストーリー・星・正答率・ガイド既読・アイコンを1本の文字列(PAKU2…。id を3バイトの指紋にして持つので短い。ふつうは数十〜数百文字、全部遊び終えても約1000文字。[transferV2.ts](src/features/transfer/transferV2.ts)。以前の PAKU1 も読める)にして、新しい端末で貼り付けて復元する(読み取り→中身の確認→上書き)。**QRコード**でも受け渡せる(発行画面に表示。復元画面の「📷 カメラで読み取る」「🖼 写真から」で読み取り、そのまま復元へ。qrcode-generator と jsQR を使い、端末の中だけで処理する)。サーバーは使わない。ランキング参加は移らない(匿名の認証が端末ごとのため)
- **保存の失敗**([safeStorage.ts](src/lib/safeStorage.ts)): 端末への保存に失敗しても(容量いっぱい・プライベートブラウズ)、例外で止まらず、画面上部に「保存できていないよ。ひきつぎコードでひかえて」と出す。保存内容が壊れていたら、別のキー(`…:corrupt`)へ退避して、最初の状態から始める
- **ランキングの不正対策**([scoreLimits.ts](src/features/ranking/scoreLimits.ts)・[firestore.rules](firestore.rules)): 得点は端末が計算して送るので、答えあわせをサーバーで行う仕組み(Cloud Functions=有料プラン)がない間は、完全には防げない。代わりに、ルールで「新規作成は5000点まで」「1回の更新で増やせるのは 10 + 8点/秒×経過秒数、かつ2万点まで」「減らせない」「上限20万」を強制する。端末側も同じ上限を守り、たまった得点は少しずつ送る(値は、ルールと必ず同じにする。テストで照合)
- **読み込みの軽量化**: 日本語フォントを使う文字だけに絞った版(約0.4MB。元は約1.9MB。[subset-fonts.py](scripts/subset-fonts.py)。文字を足したら再実行。[fontCoverage.test.ts](src/data/fontCoverage.test.ts)が知らせる)、Firebase(約0.7MB)は最初の保存に含めず、ランキングを開くときに読み込む(初回の保存: 約3.6MB → 約1.5MB)
- **会話シーン**: 画面のどこをタップしても次の台詞へ進む(ADV形式)。スキップは右上、ミュートは左上、送りの合図(▼)はテキストボックスの右下に固定(台詞の長さで位置が変わらない。テキストボックスの高さも固定)。右下には何も置かない
...
## エリアとステージの解放順(固定順)
エリアは **序章 → 1 → 2 → … → 7 の固定順でしか進めない**(自由な順番では遊べない。ストーリーが順序に依存し、成長アクセサリーの順番(①→⑦)も固定順が前提のため)。
- **エリアN+1は、エリアNの小ボスを倒すまで入れない**(`progressStore.isAreaUnlocked`)。小ボスのいない序章は、2つのステージをすべてクリアすると次(ことばの市場)が開く。最初に入れるのは序章だけ。
  エリア選択では、入れないエリアは押せず、「〇〇の小ボスを浄化すると開くよ」と出る。ステージ選択・自由練習・ことだまの書の単元の行も、入れないエリアには入れない。
- **エリア内では、通常ステージをすべてクリアして、はじめて小ボスに挑戦できる**(`Stage.requires`。通常ステージ同士に順序はない)。
- **王座の間だけ、宰相(小ボス)を倒してから、ラスボス(王様)に挑戦できる**。
- ステージの入口(ステージ画面)にも、同じ判定のガードがある(`isStageUnlocked` は、エリアの解放と `requires` の両方を見る)。
- **成長アクセサリーは、エリアクリアのときだけ付く**(エリアの全ステージのクリア=小ボス撃破。王座の間はラスボス撃破=アクセサリー⑦「王冠のかけら」)。固定順の結果、必ず①→⑦の順になる。
- 古い保存データで、前のエリアを越えていないのに先のエリアのステージがクリア済みの場合も、前のエリアを越えるまで入れない(必要なら、設定の「データを初期化する」)。

## タイトル画面のボタン
「はじめる」を主役(縦幅が大きく、やや濃いミント)にし、「ことだまの書」「ランキング」「せってい」「クレジット」は、ひと回り小さく、2×2のグリッドにまとめた(`.title-primary` / `.title-sub-buttons`)。配色はパステル基調のまま。
...
- **ミュートの固定アイコン**([MuteButton](src/components/MuteButton.tsx)): 全画面の右下に常に表示(会話シーンだけは、左上)。設定画面のミュートと同じ設定。
...
画面の上部に短い通知(「コトが せいちょうしたよ!」。上へスワイプで消せる)を出して知らせる。ずかんは最初から全ページ読めるので、ページ解放の通知は出さない([Toaster](src/components/Toaster.tsx) / [toastStore](src/app/store/toastStore.ts))。
...
- アプリがバックグラウンドに回る(ホーム画面に戻る・画面ロック)とBGMを止め、戻ると続きから鳴らす。ミュートにしたときも同じく、BGMを一時停止(pause)し、AudioContext も休ませる(音量0で鳴らし続けると、端末が「音声を再生中のアプリ」と扱い、他の音楽アプリの再生を止めてしまうため)。解除すると再開する。
