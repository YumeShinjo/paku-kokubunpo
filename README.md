...
- **正誤の判定と解説**: 解答すると「せいかい!/おしい!」のポップアップが出る(自動では消えない。タップで、ポップアップだけが閉じる)。解説は画面下の固定の枠にあり、「つぎへ」はそこにある別のボタン
- **ことばのずかん(バトル中)**: 出題中の「📖 ずかん」から、いつでも全ページを開いて確認できる(エリアのクリアを待たない)
- **ステージ背景**: 上半分に背景ビジュアル([StageVisual](src/components/ScreenBackground.tsx))、下半分に問題・UI。素材がなければ、エリアの色の仮の帯。高さは `--stage-visual-height`
- **会話シーン**: 画面のどこをタップしても次の台詞へ進む(ADV形式)。スキップだけがボタンとして残る
