...
- **アイコンの設定**は「せってい」だけ。ランキングには、選んだアイコンが出るだけで、選ぶ操作はない
- **ことばのずかん(バトル中)**: 出題中の「📖 ずかん」から、いつでも全ページを開いて確認できる(エリアのクリアを待たない)
- **ステージ背景**([StageVisual](src/components/ScreenBackground.tsx)): ボス戦は、上半分に背景+ボスを大きく(`--stage-visual-tall-height`)。通常ステージは、高さ4.5remの背景バー(`--stage-visual-height`。上端基準で切り出し、下端は背景色へフェード。エリアのテーマの小さなモチーフ(いまは絵文字の仮表示。[areaTheme.ts](src/data/areaTheme.ts))を重ねる)にとどめ、問題(特に仕分けのカゴ)の場所を優先する。素材がなければ、エリアの色の仮の帯
