...
| `boss/subboss-base` | 小ボスの共通ベース | 小ボス戦のパネル |
| `boss/subboss-<エリアid>` | 役職ごとの装飾差分(ベースと同じサイズの透過PNG) | 小ボス戦のパネル(ベースの上に重ねる) |
...
- 画像は1枚 500KB 以内を目安に(初回読み込みとオフライン保存の量が増えるため)。
...
| `explore` | エリア選択・ステージ選択・ストーリー | title |
