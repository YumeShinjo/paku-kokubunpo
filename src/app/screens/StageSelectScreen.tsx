...
export function StageSelectScreen({ areaId }: { areaId: string }) {
  ...
  const requirementLabel = (stageId: string) => {
    ...
    return label ? `${label} ${required.title}` : required.title;
  };
        ...
        {stages.map((stage) => {
          ...
          return (
            <li key={stage.id}>
              ...
              >
                <strong>{label ? `${label}: ${stage.title}` : stage.title}</strong>
                <span>{stageQuestionCount(stage)}問</span>
                {isStageCleared(stage.id) && <span> ✓クリア済み</span>}
                {resumable && (
                ...
                {!unlocked && (
                  <span className="stage-locked">
                    🔒 {(stage.requires ?? []).map(requirementLabel).join("・")}を浄化すると挑戦できるよ
                  </span>
                )}
              </button>
              {resumable && unlocked && (
